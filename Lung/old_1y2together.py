...
def create_MidV_EndInHale_EndExHale(list_exam_full, struct_to_analyze,index):
    ...
    for i in list_exam_full:
        ...
        center_roi_x.append(case.PatientModel.StructureSets[str(i)].RoiGeometries[struct_to_analyze].GetCenterOfRoi().x)
        center_roi_y.append(case.PatientModel.StructureSets[str(i)].RoiGeometries[struct_to_analyze].GetCenterOfRoi().y)
        center_roi_z.append(case.PatientModel.StructureSets[str(i)].RoiGeometries[struct_to_analyze].GetCenterOfRoi().z)
...
for ct_group in phases_groups:
    ...
    case.MapRoiGeometriesDeformably(RoiGeometryNames=[ctv_name], 
                                    CreateNewRois=False,
                                    StructureRegistrationGroupNames = ["HybridDefReg_CT" + index]*len(ct_phases_raw),
                                    ReferenceExaminationNames = [ct_ref]*len(ct_phases_raw),
                                    TargetExaminationNames=ct_phases_raw,
                                    ReverseMapping=False,
                                    AbortWhenBadDisplacementField=False)

    MidV, EndInH, EndExH = create_MidV_EndInHale_EndExHale(ct_phases_raw,ctv_name,index)
    ...
    all_rois = [x.OfRoi.Name for x in case.PatientModel.StructureSets[ct_ref].RoiGeometries]
    ...
    for ct in ct_phases_renamed:
        if not case.PatientModel.StructureSets[ct].RoiGeometries["BODY"].HasContours():
            case.PatientModel.RegionsOfInterest['BODY'].CreateExternalGeometry(Examination=case.Examinations[ct], ThresholdLevel=-250)
    
    cts_to_export = [ct_ref] + ct_phases_renamed
    for ct in cts_to_export:
    try:
        path = os.join("E:\\Clean_data_Dario",patient.Name,ct)
